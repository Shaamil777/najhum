...
export default function CropifaiHardware() {
  ...
  return (
    <section className="w-full py-16 md:py-20 bg-zinc-50 relative overflow-hidden font-sans border-b border-zinc-200">
      ...
      <Container size="xl" className="relative z-10">
        ...
        {/* Crop & Soil Sensing Systems (Turned off - code preserved) */}
        {false && (
          <>
            {/* Header */}
        ...
        <div className="relative w-full max-w-5xl mx-auto mb-10">
          ...
          <div className="text-center mb-14 md:mb-16">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-zinc-900 tracking-tight">Our Smart Ecosystem</h2>
          </div>

          {/* Central Line (Desktop) / Side Line (Mobile) */}
          <div className="absolute left-8 md:left-1/2 top-28 bottom-0 w-[2px] bg-zinc-200 md:-translate-x-1/2 rounded-full" />

          {timelineProducts.map((prod, idx) => {
            const isEven = idx % 2 === 0;
            return (
              <div key={idx} className="relative flex flex-col md:flex-row items-center justify-center gap-8 md:gap-0 mb-16 md:mb-20 last:mb-0 group">
                ...
                {/* Image Block */}
                <div className={`w-full md:w-1/2 pl-16 pr-4 md:px-12 flex ${isEven ? 'md:justify-end' : 'md:justify-start'} order-1 ${isEven ? 'md:order-1' : 'md:order-2'}`}>
                   <motion.div 
                     initial={{ opacity: 0, x: isEven ? -40 : 40 }}
                     whileInView={{ opacity: 1, x: 0 }}
                     viewport={{ once: true, margin: "-80px" }}
                     className="relative w-full max-w-[280px] md:max-w-[320px] aspect-square bg-white rounded-2xl md:rounded-3xl border border-zinc-200/90 shadow-md shadow-zinc-200/50 flex items-center justify-center p-6 overflow-hidden group-hover:border-primary/40 transition-colors"
                   >
                     <Image src={prod.image} alt={prod.name} fill sizes="(max-width: 768px) 100vw, 320px" className="object-contain p-5 md:p-6 transition-transform duration-700 group-hover:scale-105" />
                   </motion.div>
                </div>

                {/* Text Block */}
                <div className={`w-full md:w-1/2 pl-16 pr-4 md:px-12 flex flex-col justify-center order-2 ${isEven ? 'md:order-2 md:text-left' : 'md:order-1 md:text-right md:items-end'}`}>
                   <motion.div 
                     initial={{ opacity: 0, x: isEven ? 40 : -40 }}
                     whileInView={{ opacity: 1, x: 0 }}
                     viewport={{ once: true, margin: "-80px" }}
                   ...
                   >
                     <span className="text-primary font-bold tracking-widest text-[11px] uppercase mb-2 block">0{idx + 1}</span>
                     <h3 className="text-xl lg:text-2xl font-black text-zinc-900 mb-2.5 leading-tight tracking-tight">{prod.name}</h3>
                     <p className="text-sm md:text-[15px] text-zinc-500 mb-5 leading-relaxed">{prod.description}</p>
                     <ul className="space-y-2.5 inline-flex flex-col text-left">
                       {prod.features.map((f, i) => (
                         <li key={i} className={`flex items-center gap-2.5 text-xs md:text-sm text-zinc-700 font-semibold ${!isEven ? 'md:flex-row-reverse md:text-right' : ''}`}>
